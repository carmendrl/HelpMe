...
export class StudentDashboardComponent implements OnInit {
  ...
  private sessions : ApiResponse<LabSession[]>; //all labsessions
  private myQuestions : Question[];
  private invalidId: boolean; //is the sessionID valid?
  ...
  closeResult: string; //why did the modal close
  ...
  constructor(private route: ActivatedRoute, private labSessionService : LabSessionService, audioService: AudioService, private userService: UserService, private questionService: QuestionService,private modalService: NgbModal,
    private router : Router) {
      ...
      this.token = this.route.snapshot.queryParamMap.get('token'); //get session token from url
      this.myQuestions = [];
    }
    ...
    joinSess(content){
      ...
      //check if they labsession has started yet
      checkIfStarted(id: string, content){
        this.currentDate = new Date();
        this.labSessionService.getStartDate(this.token).subscribe(r =>
          {
            let tenBefore = new Date(r.toString()); //allow students to join ten minutes before
      ...
      //updates the reason that the modal closed
